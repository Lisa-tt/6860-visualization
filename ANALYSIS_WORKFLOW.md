...
- 当前源文件大小：**17.62 MB**；SHA-256：`bb83c9ed635fe04d26cc55563a1094a9dd99e99e61357099dbf959b12d3111bd`。
- 教师增加的空间与交通字段包括 Geometry、最近 train/bus station 及相应距离，因此这些字段需要额外的数据质量审查。
- 当前快照包含 **17,663** 个服务，Centre-Based Care 为 **17,246** 个，Family Day Care 为 **417** 个。
...
- DataPack 大小：**5.21 MB**；SHA-256：`2847bb0a20d13c2291c6902063bab3fa117bb7ec133f81a7bc4b317db8287897`。
- 纳入常规八州/领地 RA 的儿童人口为 **4,316,175**。
...
- 数据行数：**17,663**。
- 完全重复行：**0**；重复 Service Approval Number：**0**。
- 有 recognised overall rating：**16,336**，占 **92.5%**。
- 有效坐标并成功匹配 ABS RA：**17,652**，占 **99.9%**。
- Overall rating 缺失率：**7.5%**。
- Annual weekly hours 缺失率：**39.3%**。
- 超过 500 km、需要检查的 bus/train 距离记录：**114**。这些观测经偏远地区核查后保留，不使用 IQR 规则机械删除。
- 存在重叠 school-term sessions 并完成区间合并的记录：**841**。
...
4. 可用于主要 annual-hours 分析的记录为 **10,716**；完整模型样本为 **10,708**。
...
| 分析样本 | 服务数 | 占源数据 |
|---|---:|---:|
| Source services | 17,663 | 100.0% |
| Recognised overall rating | 16,336 | 92.5% |
| Matched ABS remoteness | 17,652 | 99.9% |
| Quality model complete cases | 16,326 | 92.4% |
| Centre-Based capacity analysis | 17,233 | 97.6% |
| Annual-hours analysis | 10,708 | 60.6% |
...
|---|---|---:|---:|---:|---:|---:|---:|---:|---:|
| Bus distance (km) | All services | 17,663 | 0.0 | 32.1 | 72.8 | 4.8 | 10.2 | 23.2 | 76.2 |
| Train distance (km) | All services | 17,663 | 0.0 | 11.4 | 44.6 | 1.1 | 2.5 | 6.3 | 20.6 |
| Nearest listed transport (km) | All services | 17,663 | 0.0 | 9.5 | 34.4 | 1.0 | 2.3 | 5.4 | 16.0 |
| Approved places | Centre-Based Care | 17,244 | 0.0 | 68.8 | 40.2 | 40.0 | 60.0 | 90.0 | 120.0 |
| Annual operating hours/week | All services | 10,716 | 39.3 | 54.0 | 13.6 | 52.5 | 55.0 | 60.0 | 60.0 |
...
Coverage 使用 Centre-Based approved places per 1,000 children aged 0-13；全国 benchmark 为 **274.6**。同时报告 services per 1,000，以区分机构数量和机构规模。
...
1. **地理结构明显。** Nearest listed transport 的中位数从 Major Cities 的 **1.89 km** 上升到 Very Remote 的 **230.03 km**。
2. **质量差异存在，但属于未调整比较。** NT 的 Below-NQS share 最高，为 **21.9%**；Remoteness groups 从 7.8% 到 30.6% 不等。
3. **全国最弱质量领域为 QA1。** 其 Below-NQS rate 为 **6.2%**。
4. **Raw quality-accessibility gradient 经调整后明显减弱。** Bus distance 每翻倍 OR 为 **0.965**（95% CI 0.934-0.998）；Train OR 为 **1.003**（95% CI 0.973-1.035）。
5. **Capacity 的地理差异比 opening-hours association 更清楚。** Centre-Based median capacity 从 66 places 降至 26 places；adjusted bus-distance effect 为 **-1.64%** per doubling。
6. **人口标准化后 capacity intensity 仍呈梯度。** Approved places per 1,000 children 从 Major Cities 的 **293.9** 降至 Very Remote 的 **141.2**。
7. **共有 12 个 State x Remoteness groups 同时触发四项增强筛查规则。** 这些是进一步调查对象，不是确定性的资源优先级。
...
1. **对 12 个增强筛查群体开展第二阶段验证。** 核查 rating denominator、具体 QA 弱项、当地 service mix、capacity 使用率和真实出行条件，再决定是否需要政策干预。
...
4. **针对 QA1 和地理差异开展质量支持。** 先分析该 QA 在高风险州和偏远地区的具体标准项，再设计 guidance、training 或 regulatory follow-up。
