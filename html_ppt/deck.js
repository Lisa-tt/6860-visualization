(() => {
  ...
  const chapterButtons = Array.from(document.querySelectorAll('.chapter-nav button[data-chapter-start]'));
  ...
  function updateChapterNav() {
    ...
    chapterButtons.forEach(button => {
      const start = Number(button.dataset.chapterStart);
      const end = Number(button.dataset.chapterEnd);
      const active = slideNumber >= start && slideNumber <= end;
  ...
  chapterButtons.forEach(button => {
    button.addEventListener('click', () => showSlide(Number(button.dataset.chapterStart) - 1));
  });
